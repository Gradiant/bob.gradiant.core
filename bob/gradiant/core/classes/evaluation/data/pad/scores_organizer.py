...
def get_score_container_filtered_by_type_attack(list_experiment_result, type_attack, attacks_correspondences):
    ...
    for experiment_result in list_experiment_result:
        scores_genuine = experiment_result.scores[experiment_result.labels == 0]
        ...
        if type_attack == ALL_ATTACKS_NAME:
            reference_labels = np.unique(experiment_result.labels)
            for label in reference_labels.tolist():
                if label != 0:
                    scores_attack = experiment_result.scores[experiment_result.labels == label]
            ...
            scores_attack = experiment_result.scores[experiment_result.labels == index_label]
            scores = np.append(scores, scores_attack)
...
class ScoresOrganizer(object):
    ...
    def run(list_experiment_result):
        ...
        list_attacks = [ALL_ATTACKS_NAME] + attacks_correspondences.keys()

        parameters_grid = list(ParameterGrid({ 'type_attack': list_attacks, 'framerate': list_framerate, 'time_capture': list_time_capture}))
        for parameters in parameters_grid:
            pad_info = PadInfo(parameters['type_attack'],parameters['framerate'], parameters['time_capture'])

            filtered_list_experiment_result = filter_by_framerate_and_time_capture(list_experiment_result, parameters['framerate'], parameters['time_capture'])
            scores_container = get_score_container_filtered_by_type_attack(filtered_list_experiment_result, parameters['type_attack'], attacks_correspondences)
