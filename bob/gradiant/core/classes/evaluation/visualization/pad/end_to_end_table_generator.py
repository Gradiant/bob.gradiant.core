...
class EndToEndTableGenerator(TableGenerator):
    ...
    def run(self, name_algorithm = 'name_algorithm'):
        ...
        table_dic["Accuracy"] = []
        table_dic["Framerate"] = []
        table_dic["Time time of acquisition"] = []
        table_dic["Time response"] = []
        table_dic["Total process/CPU time"] = []
        table_dic["Process/CPU time per frame"] = []
        table_dic["CPU Usage"] = []
        ...
        labels_list = np.array(self.dict_performance["labels_list"])
        benchmark_labels_list = np.array(self.dict_performance["benchmark_labels_list"])

        table_dic["Method"].append(name_algorithm)
        table_dic["Accuracy"].append(str(round(100 * np.sum(labels_list == benchmark_labels_list) / len(labels_list), 2))+ " %")
        ...
        table_dic["Time response"].append(str(round(np.mean(np.array(time_of_delay_list)), 2)) + "+-" +
                                          str(round(np.std(np.array(time_of_delay_list)), 2))+ " ms")

        table_dic["Total process/CPU time"].append(str(round(np.mean(np.array(cpu_time_list)), 2)) + "+-" +
                                                   str(round(np.std(np.array(cpu_time_list)), 2)) + " ms")

        table_dic["Process/CPU time per frame"].append(str(round(np.mean(np.array(cpu_time_list)/np.array(processed_frames)), 2)) + "+-" +
                                                       str(round(np.std(np.array(cpu_time_list)/np.array(processed_frames)), 2)) + " ms")
        ...
        data_frame = data_frame[
            ['Method', 'Accuracy', 'Framerate', 'Time time of acquisition', 'Time response', 'Total process/CPU time', 'Process/CPU time per frame', 'CPU Usage']]
        # todo: pandas style is not working, check for updates!
        # table_df.style.apply(self.highlight_max, color='darkorange', subset=['t_resp (mean)', 't_resp (std)', 'cpu (mean)', 'cpu (std)'])
